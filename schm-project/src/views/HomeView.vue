        ...
        <div id="destination-instance-header" class="contract-block-header d-flex">
          ...
          <div class="">
            <Button
              ...
              @click="OpenEditContractForm(contract)"
            >
              Edit
            </Button>
          </div>
          <div style="margin-left:0.5rem;">
            <div
              class="btn btn-undo"
              v-if="!contract.completed && !contract.cancelled"
              @click="useContractStore().deleteContract(contract.id)"
            >
              <svg style="width:1rem; fill:darkred;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
          ...
          <div id="destination-instance-payload" class="d-flex">
            ...
            <div class="ml-auto">
              <div style="width: auto; text-wrap: nowrap">Status: {{ payload.status }}</div>
                ...
                <!--create a flex box of up to six container sizes and counts-->
                  <div class="d-flex flex-wrap gap-1 my-3">
                    <div
                      v-for="(container, index) in calculatePayload(payload.quantity ?? 0, contract.containerSize)"
                      :key="index"
